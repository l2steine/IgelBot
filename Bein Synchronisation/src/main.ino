...
double LeadVornesollwert = (3600); //Wenn es hier definiert werden soll, Serial.ParseInt im loop löschen
double LeadHintensollwert;
double FollowHintensollwert;
double FollowVornesollwert;
...
PID VLReg(&VLRegIn, &VLRegOut, &LeadVornesollwert, Kp, Ki, Kd, DIRECT);
PID VRReg(&VRRegIn, &VRRegOut, &LeadHintensollwert, Kp, Ki, Kd, DIRECT);
PID HLReg(&HLRegIn, &HLRegOut, &FollowHintensollwert, Kp, Ki, Kd, DIRECT);
PID HRReg(&HRRegIn, &HRRegOut, &FollowVornesollwert, Kp, Ki, Kd, DIRECT);
...
while(encoder2Pos<45)  {     //Serial.println("Rechts auf Startposition...");
...
while(encoder3Pos<345)  {     //Serial.println("Rechts auf Startposition...");
...
while(encoder4Pos<205)  {     //Serial.println("Rechts auf Startposition...");
...
LeadHintensollwert = encoder1Pos - 180;         //Sollwerte der Regler definieren
FollowVornesollwert = encoder1Pos;
FollowHintensollwert = encoder2Pos;

VLReg.Compute();
VRReg.Compute();
HLReg.Compute();
HRReg.Compute();
//sollwert = Serial.parseInt();     //Sollwert einstellen, funktioniert jedoch noch nicht
delay(100);
